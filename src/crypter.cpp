...
{
    {
        ...
        if (CWalletDB(pwalletMain->strWalletFile).ReadCurrentSeedHash(hashSeed)) {
            ...
            if (!GetDeterministicSeed(hashSeed, nSeed)) {
                return error("Failed to read mBEANS seed from DB. Wallet is probably corrupt.");
            }
            pwalletMain->zwalletMain->SetMasterSeed(nSeed, false);
        } else {
            // First time this wallet has been unlocked with dmBEANS
...
bool CCryptoKeyStore::AddDeterministicSeed(const uint256& seed)
{
    ...
    if(IsCrypted()) {
        ...
        strErr = "save mbeansseed to wallet";
    }
                //the use case for this is no password set seed, mint dmBEANS,
