...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that mbeansspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MOCHA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for mBEANS send
...
public:
    ...
    std::string GetUniqueWalletBackupName(bool fmbeansAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CmBEANSTracker> mbeansTracker;
    ...
    void setZWallet(CmBEANSWallet* zwallet);
    CmBEANSWallet* getZWallet();
    ...
    /// Get 10000 MOCHA output and keys which can be used for the Masternode
    ...
    /** mBEANS reset */
    boost::signals2::signal<void()> NotifymBEANSReset;
