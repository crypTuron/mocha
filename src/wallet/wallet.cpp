...
#include "mbeans/mbeanswallet.h"
#include "mbeans/mbeanstracker.h"
...
{
    return mbeansTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd mbeans
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use mBEANS in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = mbeansTracker->GetMints(true);
    ...
    return mbeansTracker->GetBalance(false, false);
}
...
{
    return mbeansTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = mbeansTracker->ListMints(true, true, true);
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    /* Disable mBEANS Staking
    //mBEANS
    if ((GetBoolArg("-mbeansstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update mBEANS set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = mbeansTracker->ListMints(true, true, fUpdate);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 MOCHA collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 MOCHA collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 MOCHA.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 MOCHA.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-mocha-address
                    CScript scriptChange;
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld mBEANS. Current percentage of mBEANS: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // mBEANS are integers > 0, so we can't mint 10% of 9 MOCHA
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting mBEANS\n", nBalance);
        return;
    }

    // Percentage of mBEANS we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing mBEANS (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // mBEANS amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // mBEANS amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed mBEANS We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough MOCHA to mint this denomination
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new mbeans output", __func__);
    ...
    // Sign if these are mocha outputs - NOTE that mBEANS outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent mBEANS
        ...
        if(!mbeansTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = mbeansTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!mbeansTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(mbeansTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = mbeansTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = mbeansTracker->GetSpendCache(meta.hashStake);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set mBEANS privkey mint version=%d", __func__, nVersion);
        ...
        bool isPublicSpend)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of mBEANS"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one mBEANS public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the mBEANS used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = mbeansTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the mBEANS mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to mocha address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!mbeansTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            mbeansTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!mbeansTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            mbeansTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fmbeansAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fmbeansAuto ? "-autombeansbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZPivBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autombeansbackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autombeansbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autombeansbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-mbeansbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-mbeansbackuppath", ""));
...
)
{
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            mbeansTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        mbeansTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = mbeansTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending mBEANS was successful
...
{
    CMintMeta meta;
    if (!mbeansTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (mbeansTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (mbeansTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = mbeansTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return mbeansTracker->UpdateState(meta);
    } else {
...
{
    // We don't even need to worry about this code.. no mBEANS.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("mocha-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = mbeansTracker->GetSpendCache(serialHash);
...
void CWallet::setZWallet(CmBEANSWallet* zwallet)
{
    zwalletMain = zwallet;
    mbeansTracker = std::unique_ptr<CmBEANSTracker>(new CmBEANSTracker(strWalletFile));
}

CmBEANSWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
