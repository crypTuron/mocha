...
CmBEANSTracker::CmBEANSTracker(std::string strWalletFile)
{
...
CmBEANSTracker::~CmBEANSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CmBEANSTracker::Init()
{
...
bool CmBEANSTracker::Archive(CMintMeta& meta)
{
...
bool CmBEANSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CmBEANSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CmBEANSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CmBEANSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CmBEANSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CmBEANSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CmBEANSTracker::GetSerialHashes()
{
...
CAmount CmBEANSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CmBEANSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CmBEANSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CmBEANSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CmBEANSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CmBEANSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CmBEANSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CmBEANSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CmBEANSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CmBEANSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CmBEANSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CmBEANSWallet* mBEANSWallet)
{
    bool ismBEANSWalletInitialized = (NULL != mBEANSWallet);
    CMintMeta meta;
    ...
    if (! ismBEANSWalletInitialized)
        mBEANSWallet = new CmBEANSWallet(strWalletFile);
    meta.isSeedCorrect = mBEANSWallet->CheckSeed(dMint);
    if (! ismBEANSWalletInitialized)
        delete mBEANSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CmBEANSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CmBEANSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CmBEANSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CmBEANSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CmBEANSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CmBEANSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CmBEANSWallet* mBEANSWallet = new CmBEANSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, mBEANSWallet);
        }
        delete mBEANSWallet;
        LogPrint("zero", "%s: added %d dmbeans from DB\n", __func__, listDeterministicDB.size());
    }
