...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/mocha.conf are parsed in qt/mocha.cpp's main()
    ...
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        ...
        if (mapArgs.count("-version")) {
            ...
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  mochad [options]                     " + _("Start Pivx Core Daemon") + "\n";
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "mocha:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in mochad anymore. Use the mocha-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
        fDaemon = GetBoolArg("-daemon", false);
        if (fDaemon) {
            fprintf(stdout, "MOCHA server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect mochad signal handlers
    noui_connect();
