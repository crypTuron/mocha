...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total mocha balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"mBEANSsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 mBEANS denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 mBEANS denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 mBEANS denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 mBEANS denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 mBEANS denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 mBEANS denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 mBEANS denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 mBEANS denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all mBEANS denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in mocha/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in mocha/kb\n"
    ...
    UniValue mbeansObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        mbeansObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    mbeansObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("mBEANSsupply", mbeansObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"mochaaddress\"\n"
            "\nReturn information about the given mocha address.\n"

            "\nArguments:\n"
            "1. \"mochaaddress\"     (string, required) The mocha address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mochaaddress\",    (string) The mocha address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for MOCHA cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MOCHA address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are mocha addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) mocha address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"mochaaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"mochaaddress\"  (string, required) The mocha address to use for the signature.\n"
