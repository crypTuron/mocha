...
#include "qt/mocha/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of only sharing a MOCHA address, you can create a Payment Request message which bundles up more information than is contained in just a MOCHA address."));
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " MOCHA";
        } else {
