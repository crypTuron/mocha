...
#include "qt/mocha/forms/ui_dashboardwidget.h"
#include "qt/mocha/sendconfirmdialog.h"
#include "qt/mocha/txrow.h"
...
DashboardWidget::DashboardWidget(MOCHAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelZpiv, "text-chart-mbeans");
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("MOCHA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mbeans = (pair.second != 0) ? pair.second / 100000000 : 0;
        ...
        chartData->valueszPiv.append(mbeans);

        int max = std::max(piv, mbeans);
...
void DashboardWidget::onChartRefreshed() {
    ...
    if (chartData->totalPiv > 0 || chartData->totalZpiv > 0) {
        ...
        setCssProperty(ui->labelAmountZpiv, "text-stake-mbeans");
    } else {
        setCssProperty(ui->labelAmountPiv, "text-stake-piv-disable");
        setCssProperty(ui->labelAmountZpiv, "text-stake-mbeans-disable");
    }
