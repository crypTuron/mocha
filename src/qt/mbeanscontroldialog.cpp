...
#include "ui_mbeanscontroldialog.h"
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select mBEANS Denominations to Spend"));
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->mbeansTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->mbeansTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend mBEANS.";
            else if (!mint.isSeedCorrect)
                strReason = "The mBEANS seed used to mint this mBEANS is not the same as currently hold in the wallet";
            else
