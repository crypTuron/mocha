...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MOCHA);
    ...
    if (settings.contains("nAnonymizePivxAmount") || reset)
        SoftSetArg("-anonymizemochaamount", settings.value("nAnonymizePivxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mocha.cpp during shotdown to also remove the window positions
