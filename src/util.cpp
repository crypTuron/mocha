...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "mocha" is a composite category enabling all MOCHA-related debug output
            if (ptrCategory->count(std::string("mocha"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MOCHA
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MOCHA
// Mac: ~/Library/Application Support/MOCHA
// Unix: ~/.mocha
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MOCHA";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "mocha.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mocha.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mocha.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "mochad.pid"));
