...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : mBEANS stake block is less than allowed start height", __func__);

    if (CZPivStake* mBEANS = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = mBEANS->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with mBEANS stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : mBEANS stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(mBEANS->GetValue()));
        if (nChecksum200 != mBEANS->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, mBEANS->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked mBEANS fails context checks", __func__);
    } else {
