...
MOCHA Core version *version* is now available from:  <https://github.com/mocha-project/mocha/releases>
...
You can read more details about this extensive work in ([PR #954](https://github.com/MOCHA-Project/MOCHA/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/MOCHA-Project/MOCHA/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [MOCHA Wiki](https://github.com/MOCHA-Project/MOCHA/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/MOCHA-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/MOCHA-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/MOCHA-Project/MOCHA/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 MOCHA, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
[#PR1022](https://github.com/MOCHA-Project/MOCHA/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/MOCHA-Project/MOCHA/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/MOCHA-Project/MOCHA/pull/975)) and `SPORK_18` ([#PR995](https://github.com/MOCHA-Project/MOCHA/pull/995)).<br>
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-mbeans-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
- `delegated_balance` - MOCHA balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - MOCHA balance held in cold staking addresses.
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~mocha) repository, we are now offering a [Snap package](https://snapcraft.io/mocha-core) as quick way to install and update a MOCHA wallet.
