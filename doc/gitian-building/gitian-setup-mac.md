...
The build script we'll be using is contained in the MOCHA github repository ([contrib/gitian-build.py](https://github.com/mocha-project/mocha/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the MOCHA repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/MOCHA-Project/MOCHA/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, mocha-detached-sigs, and mocha GitHub repos.
