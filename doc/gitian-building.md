...
*Setup instructions for a gitian build of MOCHA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MOCHA
...
to the MOCHA GitHub release page.
...
- [Building MOCHA Core](#building-mocha-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/mocha-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/mocha-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/mocha-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/mocha-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/mocha-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/mocha-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/mocha-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@mocha.network) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/mocha-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/mocha-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/mocha-osx-*-build.assert
```
